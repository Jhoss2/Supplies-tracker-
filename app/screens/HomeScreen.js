...
import {
  ...
  ImageBackground, Alert, TextInput, Modal, Dimensions
} from 'react-native';
...
// Disposition identique à la maquette web
const ROWS = [
  ['TOUR DU SAVOIR', 'TOGUYENI', 'SALLE 15', 'SALLE 05'],                                     // 4 colonnes
  ['SALLE 04','SALLE 06','SALLE 07','SALLE 16','SALLE 17','SALLE 18',
   'SALLE 19','SALLE 21','SALLE 22','SALLE 23','SALLE 26','SALLE 27',
   'LAB B ROOM 3','LAB ROOM 1','LAB ROOM 2','LAB ROOM 3','AMPHI R.1','AMPHI R.2.A'], // 18 colonnes en 6 cols x 3 lignes
  ['AMPHI R.2.B','AMPHI R.2.C','AMPHI R.2.D','AMPHI R.4'],                                    // 4 colonnes
];
...
export default function HomeScreen({ navigation }) {
  ...
  // Rangée de boutons qui remplissent toute la largeur
  const RoomRow = ({ rooms, numCols }) => (
    <View style={styles.row}>
      {rooms.map(name => (
        <View key={name} style={[styles.cellWrap, { flex: 1 }]}>
          <RoomButton
            text={name}
  ...
  // La rangée du milieu (18 salles) : 6 colonnes × 3 lignes
  const MiddleSection = () => {
    const middle = ROWS[1];
    const chunks = [];
...
const styles = StyleSheet.create({
  ...
  content: {
    ...
    paddingVertical: 10,
    justifyContent: 'space-evenly',  // répartition verticale uniforme
  },
  row: {
    flexDirection: 'row',
    marginVertical: 3,
  ...
  amphi0Wrap: {
    width: '16.6%',   // 1/6 de largeur comme les autres salles du bas
  },
  // Modal
  modalOverlay: {
