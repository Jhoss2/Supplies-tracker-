...
export const getAllUsers = async () =>
  db.getAllAsync('SELECT * FROM users ORDER BY created_at DESC');

export const getUserById = async (id) =>
  db.getFirstAsync('SELECT * FROM users WHERE id = ?', [id]);
...
export const updateUser = async (id, data) => {
  ...
  await db.runAsync(`UPDATE users SET ${fields} WHERE id = ?`, [...Object.values(data), id]);
};
...
export const verifyUserPassword = async (userId, password) => {
  const user = await db.getFirstAsync(
    'SELECT id FROM users WHERE id = ? AND password = ?', [userId, password]
  );
  return !!user;
};
...
export const deleteBiometricCard = async (id) =>
  db.runAsync('DELETE FROM biometric_cards WHERE id = ?', [id]);
